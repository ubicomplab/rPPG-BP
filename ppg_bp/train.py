...
from dataset import UWBP_train, UWBP_val, UWBP_train_skew, UWBP_val_skew, UWBP_train_face_palm_skew, UWBP_val_face_palm_skew
...
class Trainer:
    ...
    def init_datasets(self):
        ...
        self.train_set = UWBP_train_skew(self.config)
        self.val_set = UWBP_val_skew(self.config)
    ...
    def init_model(self):
        if self.config["derivative_input"]:
            self.model = M5(n_input=6, n_output=2)
        else:
            self.model = M5(n_output=2)
    ...
    def train_loss(self, batch, epoch, step):
        ...
        if self.rank == 0:
            sys.stdout.write(f"\r[{epoch},{step}] loss {loss.item():.4f} l1 loss {l1_loss.item():.4f}")
            sys.stdout.flush()
