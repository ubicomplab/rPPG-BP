...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--save_path", type=str, default="./save_folder", help="save path of processed mat")
    parser.add_argument("-f", "--face", type=bool, help="True for face process / False for finger process")
    ...
    all_mat_folder = "/gscratch/ubicomp/xliu0/data3/mnt/Datasets/UWMedicine/ProcessedNoVideoDataV2/*.npy"
    ...
    figure_savepath = "/gscratch/ubicomp/cm74/bp/bp_preprocess/test_figure_plots_v2_palm"
    ...
    for mat_path in all_mat_path:
        ...
        if opt.face:
            face_ppg = (mat_data["face_patch_green"][1] + mat_data["face_patch_green"][2] + mat_data["face_patch_green"][0] + mat_data["face_patch_green"][3]
                    + mat_data["face_patch_green"][8] + mat_data["face_patch_green"][9] + mat_data["face_patch_green"][10] + mat_data["face_patch_green"][11]
                    + mat_data["face_patch_green"][12] + mat_data["face_patch_green"][13] + mat_data["face_patch_green"][14] + mat_data["face_patch_green"][15]) / 12
        else:

            face_ppg = finger_ppg
        # face_ppg = 1 - mat_data["gt_ppg"]
        
        # face_ppg = mat_data["raw_palm_green"]
        # face_ppg = np.reshape(face_ppg, (face_ppg.shape[0]))
        # finger_ppg = mat_data["ppg"][0]
        # finger_ppg = mat_data["ppg"][0]
        # face_ppg = mat_data["ppg_face"][0]
        # face_ppg = mat_data["ppg_palm"][0]
        # face_ppg = 1 - mat_data["ppg_palm"][0]
        ...
        try:
            ...
            if visualize_segmentation:
                ...
                plt.savefig(os.path.join(figure_savepath, video_name + "_ppg_visualization.png"))
                plt.close()
