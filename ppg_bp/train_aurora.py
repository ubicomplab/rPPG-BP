...
from model_resnet import ResNet1D
from dataset_aurora import MS_train_selected_demo, MS_val_selected_demo
...
class Trainer:
    ...
    def init_datasets(self):
        ...
        self.train_set = MS_train_selected_demo(self.config)
        self.val_set = MS_val_selected_demo(self.config)
    ...
    def init_model(self):
        if self.config["derivative_input"]:
            ...
            # self.model = ResNet1D(in_channels=1, base_filters=64, kernel_size=3, stride=1, groups=1, n_block=8, n_classes=2)
    ...
    def train_loss(self, batch, epoch, step):
        ...
        # print(bp_weight)
        # l1_loss = bp_weight * self.l2_criterion(bp_predict, bp)
        # loss = l1_loss.mean()

        l1_loss = self.l2_criterion(bp_predict, bp)
        # l1_loss = self.l1_criterion(bp_predict, bp)
        loss = l1_loss
